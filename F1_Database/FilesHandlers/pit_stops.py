    ...
    try:
        ...
        with open(csv_file, 'r', encoding='utf-8') as file:
            ...
            for row in csv_data:
                ...
                insert_query = "INSERT INTO pit_stops(raceId,driverId,stop,lap," \
                                   "time,duration,milliseconds) VALUES (%s, %s, %s, %s, %s, %s, %s)"
                ...
                values = (int(row[0]) if row[0] is not None else None, int(row[1]) if row[1] is not None else None, int(row[2]) if row[2] is not None else None,
                      int(row[3]) if row[3] is not None else None, datetime.strptime(row[4], "%H:%M:%S").time(),
                          parse_time(row[5]) if row[5] is not None else None, int(row[6]) if row[6] is not None else None)  # Aquí asume que los valores están en la columna 1 y columna 2
