    ...
    try:
        ...
        with open(csv_file, 'r', encoding='utf-8') as file:
            ...
            for row in csv_data:
                ...
                insert_query = "INSERT INTO qualifying(qualifyId,raceId,driverId,constructorId,number,position,q1,q2,q3) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
                ...
                values = (int(row[0]) if row[0] is not None else None, int(row[1]) if row[1] is not None else None, int(row[2]) if row[2] is not None else None,
                      int(row[3]) if row[3] is not None else None, int(row[4]) if row[4] is not None else None,
                      int(row[5]) if row[5] is not None else None, datetime.strptime(row[6], "%M:%S.%f").time() if (row[6] is not None and row[6] != '') else None,
                            datetime.strptime(row[7], "%M:%S.%f").time() if (row[7] is not None and row[7] != '') else None,
                            datetime.strptime(row[8], "%M:%S.%f").time() if (row[8] is not None and row[8] != '') else None)
