    ...
    try:
        ...
        with open(csv_file, 'r', encoding='utf-8') as file:
            ...
            for row in csv_data:
                ...
                insert_query = "INSERT INTO constructor_standings(constructorStandingsId,raceId,constructorId,points,position,positionText,wins) VALUES (%s, %s, %s, %s, %s, %s, %s)"
                ...
                values = (int(row[0]) if row[0] is not None else None, int(row[1]) if row[1] is not None else None, int(row[2]) if row[2] is not None else None,
                          float(row[3]) if row[3] is not None else None, int(row[4]) if row[4] is not None else None, row[5], int(row[6]) if row[6] is not None else None)  # Aquí asume que los valores están en la columna 1 y columna 2
